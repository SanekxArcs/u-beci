...
import { CalendarIcon, PlusCircle, Pencil, Trash2, Copy, RefreshCcw, CalendarArrowDown, CalendarArrowUp, Eye, EyeClosed } from 'lucide-react'
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button onClick={() => setSortAsc((s) => !s)} variant="outline">
            {sortAsc ? <CalendarArrowDown /> : <CalendarArrowUp />}
          </Button>
          <Button className="flex-1" onClick={() => openDialog()}>
            <PlusCircle className="h-4 w-4" /> Dodaj
          </Button>
        </div>
      </div>
        ...
        <DialogContent className="sm:max-w-[600px]">
            ...
            <DialogTitle>
              {editingMenu
                ? isCopy
                  ? "Kopiuj menu dnia"
                  : "Edytuj menu dnia"
                : "Utwórz nowe menu dnia"}
            </DialogTitle>
            <DialogDescription>
              {editingMenu
                ? isCopy
                  ? "Wybierz nową datę i skopiuj pozycje menu."
                  : "Zaktualizuj datę, pozycje menu i opis dla tego menu dnia."
                : "Wybierz datę, pozycje menu i dodaj opis na ten dzień."}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave}>
              {editingMenu && !isCopy ? "Zaktualizuj menu" : "Zapisz menu"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <div className="space-y-6">
        {filteredMenus.length > 0 ? (
                ...
                <CardContent>
                  {menuItems.length > 0 ? (
                    ...
                    <p className="text-muted-foreground">
                      Brak pozycji wybranych na ten dzień.
                    </p>
                  )}
                </CardContent>
              </Card>
          ...
          <div className="text-center py-12 border rounded-lg">
            <p className="text-muted-foreground">
              Nie utworzono jeszcze żadnego menu dnia.
            </p>
          </div>
        )}
      </div>
